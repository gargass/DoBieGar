...
geny <- read.table('P_value - lista genow + pvalue official/lista_interesujacych_genow.txt')
...
for(nowotwor in nowotwory){
  ...
  nowotwor.cli_mut.V$time<-ifelse(is.na(nowotwor.cli_mut.V$patient.days_to_death),nowotwor.cli_mut.V$patient.days_to_last_followup,nowotwor.cli_mut.V$patient.days_to_death)
    ...
    for(g in 1:nrow(geny)){
      ...
      nowotwor.cli_mut.I <- merge(nowotwor.cli_mut.I, nowotwor.mutations_sub_gen.I, by.y='Tumor_Sample_Barcode', by.x='patient.bcr_patient_barcode', all=TRUE)
      ...
      nowotwor.cli_mut.V <- merge(nowotwor.cli_mut.V, nowotwor.mutations_sub_gen.V, by.y='Tumor_Sample_Barcode', by.x='patient.bcr_patient_barcode', all=TRUE)
    ...
    write.table(nowotwor.cli_mut.I, file = paste('Zbiory/', nowotwor, '/', nowotwor, '_I.txt', sep=""))
    write.table(nowotwor.cli_mut.V, file = paste('Zbiory/', nowotwor, '/', nowotwor, '_V.txt', sep=""))
}